...
export const getScreenshot = (tabName:string) => {
	switch (tabName) {
		case 'download':
			const captureElement:HTMLElement|null = document.querySelector('.capture');
...
export const iOSFix = () => {
	...
	if (isTablet) {
        const forIos = document.querySelector('.forIOS');
        if(forIos){
            forIos.classList.add('iOS');
        }
